...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
$datatable="add_knqot";
	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='apbilling')or ($tsname=='rajesh.p')or ($tsname=='sumanthpotluri')or ($tsname=='naiduys')){
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:S1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KARNATAKA WORK TO STARTED QUOTATION LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('R'.$rowCount, mb_strtoupper($row['reason'],'UTF-8'));
	$rowCount++;
$i++; }
...
header('Content-Disposition: attachment;filename="knworktobestartd.xlsx"'); //tell browser what's the file name
