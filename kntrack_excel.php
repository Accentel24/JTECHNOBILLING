...
$objPHPExcel->getActiveSheet()->mergeCells('A1:BJ1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KARNATAKA TRACKER LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$ds10="select sum(approve_amnt) as apamount,sum(transfer) as kltft,sum(gstamt) as tngst from knrequest_amnt where quet_num='$qno'";
	...
	$ds11="select transfer_date from knrequest_amnt where quet_num='$qno' ";
	...
	$yt="select approve_amnt,ac_det,transfer_date from knrequest_amnt where quet_num='$qno'";
	...
	$ds1="select * from knqot_bill where quet_num='$qno'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('AK'.$rowCount, mb_strtoupper('KN','UTF-8'));
	...
	$y25="select * from knpayment where inv_no='$invno'"; 
...
header('Content-Disposition: attachment;filename="kntrack.xlsx"'); //tell browser what's the file name
