...
// Query to fetch the quet_num column from the last row of the table
$query = "SELECT quet_num FROM add_knqot ORDER BY id DESC LIMIT 1";
// Perform the query and fetch the last row
$result = mysqli_query($link, $query);
$last_row = mysqli_fetch_assoc($result);
$last_quet_num = $last_row['quet_num'];
// Extract the last 4 digits from the quotation number
$last_4_digits = substr($last_quet_num, -4);
// Increment the last 4 digits by 1
$new_last_4_digits = intval($last_4_digits) + 1;
// Construct the updated quotation number
$new_quet_num = substr($last_quet_num, 0, -4) . str_pad($new_last_4_digits, 4, '0', STR_PAD_LEFT);
// Output the updated quotation number
// echo "The Previous Quote Number is $last_quet_num<br>";
echo "The Next Quotation Number is $new_quet_num";
?>
