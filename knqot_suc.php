...
if(isset($_POST['submit'])){
 $foldermonth= date('M');
 $folderyear= date('Y');
 $photofolderpath="knupload/".$folderyear."/".$foldermonth;
 if(!file_exists($photofolderpath))
 mkdir($photofolderpath);
// mkdir("knupload/".);
// else
...
if($qt_no1!=''){
	...
	$tr=mysqli_fetch_array($t);
	 $empemail=$tr['emp_email'];
	  ...
	  $a="INSERT INTO `add_knqot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status,img1,img2,img3,frm_type)
	 VALUES ('$store_code','$inv_date','','$tot','$tot_serv','$tot_gst','$net','$ses','$falt_no','$falt_date','$falt_desc','Ro Required','$iname1','$iname3','$iname5','$frm_type')";
	$sq=mysqli_query($link,$a);
	 $sno=mysqli_insert_id($link);
	 

$sqq=mysqli_query($link,"select `count` as ids from qutcount where state='KN' ");
	$rr=mysqli_fetch_array($sqq);
	$idd=$rr['ids'];
	$cc=$idd+1;
	 $qt_nox=24250000+1+$idd;
	 $qt_no="QJTKN$qt_nox";  
	 if($qt_no1!=''){
	     $ac=mysqli_query($link,"update add_knqot set quet_num='$qt_no1' where id='$sno'");
	 }else{
	     mysqli_query($link,"update qutcount set `count`='$cc' where state='KN'");
	$ac=mysqli_query($link,"update add_knqot set quet_num='$qt_no' where id='$sno'");
	 }
...
$productype=$_POST['productype'][$i];
$productcode=$_POST['productcode'][$i];
$remarks=$POST['remarks'][$i];

 $query = "INSERT INTO add_knqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,brand,model,remarks,productcode,productype) 
	VALUES 
 ('".addslashes($pname)."','$hsn','$gst','$uom','$qty','$gst_amnt','$price','$amnt','$date','$sno','$id','$serv_amt','$cap','$serv_amnt','$serv_code','$serv_codex','$brand','$model','$remarks','$productcode','$productype')";
...
if(isset($_POST['update'])){
 ...
 $folderyear= date('Y');
 $photofolderpath="knupload/".$folderyear."/".$foldermonth;
 if(!is_dir($photofolderpath))
 mkdir($photofolderpath, 0777, true);
	  $id=$_POST['ids'];
	    $qt_no=$_POST['qt_no']; 
    $inv_date=$_POST['inv_date'];
	$store_code=$_POST['store_code'];
...
*/
		    $v="update add_knqot set
		 
		 `quet_num`='$qt_no' ,`store_code`='$store_code' ,`tot_base`='$tot', `tot_ser`='$tot_serv', `tot_gst`='$tot_gst', `net`='$net', 
		 falt_no='$falt_no',falt_date='$falt_date',falt_desc='$falt_desc',type_of_work='$type_of_work',sub_type='$sub_type',
		 
		 img1='$iname1',img2='$iname3',img3='$iname5',img4='$iname20',img5='$iname21',img6='$iname22',   bal='$bal',gst_type='$gst_type',frm_type='$frm_type'
...
// $productcode=$_POST['productcode'][$i];
   ...
   if($id2!=''){
	   $query = "update add_knqot1 set

`desc1`='".addslashes($pname)."', `hsn`='$hsn', `gst`='$gst', `uom`='$uom', `qty`='$qty',`productype`='$productype', `gst_amnt`='$gst_amnt',`remarks`='$remarks',
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
  ...
  $query = "INSERT INTO add_knqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,`remarks`) 
	VALUES 
 ('".addslashes($pname)."','$hsn','$gst','$uom','$qty','$gst_amnt','$price','$amnt','$date','$id','$id','$serv_amt','$cap','$serv_amnt','$serv_code','$serv_codex','$remarks')";
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
}
...
$v="update `add_knbill1` set qr_code='$bar_code',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no' where id='$id'";
$res=mysqli_query($link,$v) or die(mysqli_error($link));

if($res){
	print "<script>";
	print "alert('Sucessfully Updated');";
	print "self.location='karnatakabill_list.php';";
	print "</script>";
