 ...
 $iname2 = $_FILES['img2']['name'];
	...
	$iname5 = ($iname5);
	 ...
	 if($qt_no1!=''){
	  ...
	  $a="INSERT INTO `add_tgqot`(`store_code`, `inv_date`, `quet_num`, `tot_base`, `tot_ser`, `tot_gst`, `net`,  `ses`,falt_no,falt_date,falt_desc,status,img1,img2,img3)
	...
	$cc=$idd+1;
	 ...
	 $qt_no="QJTTG$qt_nox";  
	  ...
	  if($qt_no1!=''){
	     $ac=mysqli_query($link,"update add_tgqot set quet_num='$qt_no1' where id='$sno'");
	 }else{
	     mysqli_query($link,"update qutcount set `count`='$cc' where state='TS'");
	$ac=mysqli_query($link,"update add_tgqot set quet_num='$qt_no' where id='$sno'");
	 }
...
$remarks=$POST['remarks'][$i];
 ...
 $query = "INSERT INTO add_tgqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,brand,model,`remarks`,productcode,productype) 
	VALUES 
...
$productype=$_POST['productype'][$i];
   ...
   if($id2!=''){
	   $query = "update add_tgqot1 set

`desc1`='".addslashes($pname)."', `hsn`='$hsn', `gst`='$gst', `uom`='$uom', `qty`='$qty', `gst_amnt`='$gst_amnt',`remarks`='$remarks',
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
  ...
  $query = "INSERT INTO add_tgqot1 ( `desc1`, `hsn`, `gst`, `uom`, `qty`, `gst_amnt`, `rate`, `base_amt`,`date`,`id1`,item_id,serv_fee,cap,serv_amt,serv_code,serv_cap,`remarks`) 
	VALUES 
