...
$objPHPExcel->getActiveSheet()->SetCellValue('S6', 'Advance Amount');
$objPHPExcel->getActiveSheet()->SetCellValue('T6', 'Remarks');
...
$objPHPExcel->getActiveSheet()->SetCellValue('W6', 'Rq Amount Date');
$objPHPExcel->getActiveSheet()->SetCellValue('X6', 'User');

$objPHPExcel->getActiveSheet()->getStyle("A6:X6")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
        ->getStartColor()->setRGB('800000');
...
while($row	=	$result->fetch_assoc()){
    ...
    $krid=$row['id'];
    $ac=$row['ac_det'];
    $a="select store_code,ro_no,ro_date,falt_desc,tot_base,tot_ser,tot_gst,adv_amnt,adv_amnt1,adv_amnt2,gst_type,net,bal from add_knqot where quet_num='$qot'";
	...
	$adamt="select sum(approve_amnt) as req_amnt from knrequest_amnt where quet_num='$qot'and  status='Amount Transferred' and   confirm='Yes'" ;
	...
	$tyy="select sum(approve_amnt) as req_amnt from knrequest_amnt where quet_num='$qot'   and status='' and  confirm='Yes' and ac_det='$ac'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('W'.$rowCount, mb_strtoupper($row['req_date'],'UTF-8'));
...
$objPHPExcel->getActiveSheet()->SetCellValue('F'.$tc, 'Amount');
  ...
  if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts')){
    	...
    	$y2="select distinct ac_det from knrequest_amnt where  user='$tsname' and  confirm='Yes' and status='' ";
	}
...
while($row10	=	$result10->fetch_assoc()){
    $acd=$row10['ac_det'];
	 ...
	 $r12="select sum(approve_amnt) as amt from knrequest_amnt where ac_det='$acd' and confirm='Yes' and status='' ";
