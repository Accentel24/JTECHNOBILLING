...
include 'PHPExcel-1.8/Classes/PHPExcel/Writer/Excel2007.php';
 $tsname=$_GET['user'];
	...
	or ($tsname=='sumanthpotluri')or ($tsname=='knbilling')){
    $y="select distinct quet_num,user from knrequest_amnt where confirm!='Yes' order by id desc";
    }else{
   $y="select distinct quet_num,user  from knrequest_amnt where confirm!='Yes' and user='$tsname' order by id desc";
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:U1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KARNATAKA REQUEST AMOUNT LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$tyy="select sum(req_amnt+gstamt) as rsamt from knrequest_amnt where quet_num='$qot' and confirm='Pending'";
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('S'.$rowCount, ($array2));
...
header('Content-Disposition: attachment;filename="knreqamountlist.xlsx"'); //tell browser what's the file name
