...
//	if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='vishnu')){
     $y="SELECT * FROM expenses where state='KN' ORDER BY id desc";    
    //}else{
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:G1');
 $objPHPExcel->getActiveSheet()->getStyle("A1:G1")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('800000');
...
$objPHPExcel->getActiveSheet()->getStyle("A4:G4")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('800000');
...
$sqy=mysqli_query($link,"SELECT * FROM expenses where state='KN' ORDER BY id desc");  

$objPHPExcel->getActiveSheet()->getStyle("A2:G2")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('800000');
...
header('Content-Disposition: attachment;filename="knexpenses.xlsx"'); //tell browser what's the file name
