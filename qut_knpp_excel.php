...
//$datatable="add_klqot";
	 if(($tsname=='admin') or ($tsname=='durgarao') or ($tsname=='accounts') or ($tsname=='knbilling') or ($tsname=='sumanthpotluri')){
      $y="select * from knqot_bill where status='Un Paid' ";   
    }else{
      $y="select * from knqot_bill where status='Un Paid' and user='$tsname' ";
	 }
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:W1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A4', 'KARNATAKA PAYPEMENT PENDING LIST');
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper('KN','UTF-8'));
...
header('Content-Disposition: attachment;filename="knpaymentpending.xlsx"'); //tell browser what's the file name
